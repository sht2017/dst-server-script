...
script_url='https://raw.githubusercontent.com/sht2017/dst-server-script/main/dstserver.sh'
...
Install_Dependency() {
  ...
  if [[ "${release}" != "centos" ]]; then
    if [[ "${bit}" = "x86_64" ]]; then
      ...
      sudo apt update
      sudo apt install -y lib32gcc-s1 libstdc++6 libstdc++6:i386 libcurl4-gnutls-dev:i386
    else
      sudo apt update
...
glibc_version=$(ldd --version | head -n 1 | grep -Eo "[^ ]+$" | tr -cd "[0-9]")
